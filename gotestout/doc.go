...
// events. Options allow compact or detailed views, a spinner-only live
// activity footer with auto/on/off modes for styled human output, and grouped
// failed-test, skipped-test, package-error, or captured-output sections that
// callers can disable when they want a tighter summary. In JSON mode, Render
// re-emits the raw go test events while still returning summary counts, and it
// skips the grouped human/plain summary blocks and transient activity footer.
